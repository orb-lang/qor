...
### Type Predicate: table\(tab\) \-> tab | nil, type\(tab\)
...
### getset\(synth, key\): tab: t, new: true | nil

Either retrieves the table at `key`, or assigns such a table and returns it\.

Second value is a sentinel, `true` if we created a table\.
...
Returns `i, val, val` as long as at least one table has values in the compact
region\.
